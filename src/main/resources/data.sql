...
$$
    ...
    BEGIN
        FOR i IN 1..100
            LOOP
                emp_num := (SELECT employee_number FROM employees WHERE id = i);
                hire_date_val := (SELECT hire_date FROM employees WHERE id = i);

                INSERT INTO change_logs (type, employee_number, memo, ip_address, at) VALUES ('CREATED', emp_num, '신규 직원 등록', '192.168.1.' || FLOOR(RANDOM() * 255 + 1)::INT, hire_date_val + INTERVAL '1 hour');
                INSERT INTO change_log_diffs (log_id, property_name, before, after) VALUES ((SELECT MAX(id) FROM change_logs), 'status', NULL, 'ACTIVE');
...
$$
    ...
    BEGIN
        FOR i IN 2..15
            LOOP
                update_date := '2025-08-15'::TIMESTAMPTZ + ((i / 3)::INT - 1) * INTERVAL '3 days';

                UPDATE employees SET position = '대리' WHERE id = i AND position = '사원' RETURNING employee_number INTO emp_num;

                IF emp_num IS NOT NULL THEN
                    INSERT INTO change_logs (type, employee_number, memo, ip_address, at) VALUES ('UPDATED', emp_num, '사원 -> 대리로 직급 변경', '192.168.1.' || FLOOR(RANDOM() * 255 + 1)::INT,update_date);
                    INSERT INTO change_log_diffs (log_id, property_name, before, after) VALUES ((SELECT MAX(id) FROM change_logs), 'position', '사원', '대리');
                END IF;
